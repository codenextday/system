...
extern uint32_t extern_mem_base;
extern int device_file;
...
#define FPGA_USE_KERNEL_IRQ_HANDLING 1
...
#define FPGA_IRQ_POLL_INTERVAL   20 //50 //20          /**< interrupt poll interval in microseconds */
...
/** AlteraFPGABoard_ReadReg()
  ...
  * @return the value read from the register, or FPGA_READ_ERROR if any error occured */
INLINE uint32_t AlteraFPGABoard_ReadReg(uint32_t address) {
#if FPGA_USE_MMAP
    return fpga_regs_mmap[FPGA_REG_ADDRESS_MOD(address >> 2)];
#else
    return AlteraFPGABoard_ReadBAR(FPGA_REG_BAR_NUM, FPGA_REG_ADDRESS_MOD(address));
#endif
}
...
/** AlteraFPGABoard_WriteReg()
  ...
  * @retval FPGA_RES_NODEV  FPGA board not initialized (need to call AlteraFPGABoard_Open() first)
  * @retval FPGA_RES_FAIL   could not write value */
INLINE uint32_t AlteraFPGABoard_WriteReg(uint32_t address, uint32_t data) {
#if FPGA_USE_MMAP
    fpga_regs_mmap[FPGA_REG_ADDRESS_MOD(address >> 2)] = data;
    return FPGA_RES_OK;
#else
    return AlteraFPGABoard_WriteBAR(FPGA_REG_BAR_NUM, FPGA_REG_ADDRESS_MOD(address), data);
#endif
}
...
#define AlteraFPGABoard_ForbidIRQPolling() do { } while (0)
#define AlteraFPGABoard_AllowIRQPolling()  do { } while (0)
