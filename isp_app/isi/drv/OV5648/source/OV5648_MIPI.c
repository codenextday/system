...
#else
#define Sensor_SLAVE_ADDR       (0x6cU>>1)                           /**< i2c slave address of the OV5695 camera sensor */
#define Sensor_SLAVE_ADDR2      (0x6cU>>1)
#define Sensor_SLAVE_AF_ADDR    (0x18U>>1)                           /**< i2c slave address of the OV5695 integrated AD5820 */
#define Sensor_OTP_SLAVE_ADDR   (0x6cU>>1)
#define Sensor_OTP_SLAVE_ADDR2   (0x6cU>>1)
#endif
...
#define OV5648_RST                  2
#else
#define GPIO_PIN_OV5648_AVDD        (GPIO_GROUP_1 + 4)
#define GPIO_PIN_OV5648_IO          (GPIO_GROUP_2 + 24)
#define GPIO_PIN_OV5648_DVDD        (GPIO_GROUP_2 + 10)
#define OV5648_PDN                  (GPIO_GROUP_2 + 8)
#define OV5648_RST                  (GPIO_GROUP_2 + 12)
#endif
			...
			case ISI_RES_2592_1944P15:
				{
					...
					if (set2Sensor == BOOL_TRUE) {
						TRACE( Sensor_ERROR, "%s: _set2sensor enter 1280x960*********** \n", __FUNCTION__ );
						if (res_no_chg == BOOL_FALSE) {
							TRACE( Sensor_ERROR, "%s: _1280x960 set begine ********* \n", __FUNCTION__ );
...
static void ov5648StreamOn(IsiSensorHandle_t   handle, bool_t enable)
{
...
/******************************************************************************
 * OV5648PowerCtrl() - Control OV5648 power
 *
 * @void:
 *****************************************************************************/
static RESULT OV5648PowerCtrl(uint8_t on)
{
	RESULT ret = 0;
	g_gpio_drv.pfn_export(3);

	g_gpio_drv.pfn_set_value(3, 0);
	osSleep( 10 );
	g_gpio_drv.pfn_set_value(2, 1);
	osSleep( 10 );
	g_gpio_drv.pfn_set_value(3, 1);
	osSleep( 10 );

	g_gpio_drv.pfn_set_value(2, 0);
	osSleep( 10 );
	g_gpio_drv.pfn_set_value(2, 1);
	osSleep( 10 );
	return ret;
...
{
	RESULT ret = 0;
	ret |= g_gpio_drv.pfn_set_output(GPIO_PIN_OV5648_IO, on);
	ret |= g_gpio_drv.pfn_set_output(GPIO_PIN_OV5648_AVDD, on);
	ret |= g_gpio_drv.pfn_set_output(GPIO_PIN_OV5648_DVDD, on);
	ret |= g_gpio_drv.pfn_reset(OV5648_PDN);
	return ret;
}
...
/******************************************************************************
 * OV5648Reset() - Reset OV5648
 *
 * @void:
 *****************************************************************************/
static RESULT OV5648Reset(void)
{
	return g_gpio_drv.pfn_reset(OV5648_RST);
}

/*****************************************************************************/
/**
 *          OV5648IsiSensorSetup
 *
 * @brief   Set the camera to idle state or power down
 *
 * @param   handle      OV5648 sensor instance handle
 ...
 *
 *****************************************************************************/
static RESULT OV5648IsiSensorSetup(IsiSensorHandle_t handle, bool_t on)
{
	Sensor_Context_t *pOV5648Ctx = (Sensor_Context_t *)handle;
	RESULT result = RET_SUCCESS;
	...
	pOV5648Ctx->Configured = BOOL_FALSE;

	result = OV5648PowerCtrl(on);
...
)
{
    ...
    if ( pIsiSensor != NULL )
    {
        ...
        pIsiSensor->pIsiSensorSetPowerIss               = OV5648IsiSensorSetup;
