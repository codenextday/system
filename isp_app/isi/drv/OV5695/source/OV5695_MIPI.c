...
#define Sensor_SLAVE_ADDR       (0x20U>>1)                           /**< i2c slave address of the OV5695 camera sensor */
#define Sensor_SLAVE_ADDR2      (0x20U>>1)
#define Sensor_SLAVE_AF_ADDR    (0x18U>>1)                           /**< i2c slave address of the OV5695 integrated AD5820 */
#define Sensor_OTP_SLAVE_ADDR   (0x20U>>1)
#define Sensor_OTP_SLAVE_ADDR2   (0x20U>>1)
...
#define OV5695_PWD                  3
#else
#define GPIO_PIN_OV5695_AVDD        (GPIO_GROUP_1 + 4)
#define GPIO_PIN_OV5695_IO          (GPIO_GROUP_2 + 24)
#define GPIO_PIN_OV5695_DVDD        (GPIO_GROUP_2 + 14)
#define OV5695_RST                  (GPIO_GROUP_2 + 12)
#endif
...
extern const IsiRegDescription_t Sensor_g_twolane_resolution_1920x1080[];
extern const IsiRegDescription_t Sensor_g_twolane_resolution_640x480[];
extern const IsiRegDescription_t Sensor_g_twolane_resolution_1280x960[];
...
)
{
  RESULT result = RET_SUCCESS;
	...
	TRACE( Sensor_INFO, "%s: mipi_lanes=%d, pIsiSensorCaps->Index=%d\n",
	  __FUNCTION__, mipi_lanes, pIsiSensorCaps->Index);
	...
	TRACE( Sensor_INFO, "%s how to use these caps, and where(sensor itself? or isp input if?)?\n", __FUNCTION__);
	pIsiSensorCaps->BusWidth        = ISI_BUSWIDTH_10BIT_ZZ;
	...
	pIsiSensorCaps->Conv422         = ISI_CONV422_INTER;
	pIsiSensorCaps->BPat            = ISI_BPAT_BGBGGRGR;

	pIsiSensorCaps->HPol            = ISI_HPOL_SYNCPOS;
...
const IsiSensorCaps_t Sensor_g_IsiSensorDefaultConfig =
{
  ISI_BUSWIDTH_10BIT_ZZ,         // BusWidth
...
)
{
  ...
  switch ( pConfig->BPat )            /* only ISI_BPAT_BGBGGRGR supported, no configuration needed */
  {
	...
	{
	  TRACE( Sensor_ERROR, "%s: pConfig->BPat=0x%x\n", __FUNCTION__, pConfig->BPat);
	  break;
	}
	default:
	{
...
int Sensor_get_PCLK( Sensor_Context_t *pSensorCtx, int XVCLK)
{
  ...
  int Sys_pre_div_map[] = {1, 2, 4, 1};
	...
	////////////////////////////////////////
	//0x0312:
	//bit5: pll2_div_rst_sync
	//bit4:pll2_predivp?
	//bit1~0:pll2_r_divdac
	////////////////////////////////////////
  ...
  SCLK = XVCLK * 4 / Pll2_prediv0 / Pll2_prediv2x * Pll2_multiplier /Pll2_sys_pre_div / Pll2_sys_divider2x / Sys_pre_div / sclk_pdiv;
	...
	////////////////////////////////////////
	//0x350a:long_digital_gain[13:6]
	//0x350b:long_digital_gain[5:0]
	//bit9~0: fraction
	//???????????????????????????
	////////////////////////////////////////
  temp1 = SCLK>>8;
  ...
  if(pSensorCtx->IsiSensorMipiInfo.ucMipiLanes == SUPPORT_MIPI_TWO_LANE) {
	...
	switch ( pConfig->Resolution ) {
	  ...
	  case ISI_RES_1296_972P10:
	  {
		...
		#if 0
		usLineLengthPck = 0x02a0;/*672?????*/
		if (pConfig->Resolution == ISI_RES_1296_972P30) {
		  usFrameLengthLines = 0x08b8;/*2232*/
		}else if(pConfig->Resolution == ISI_RES_1296_972P25) {
		  usFrameLengthLines = 0x0a76;/*2678*/
		}else if(pConfig->Resolution == ISI_RES_1296_972P20) {
		  usFrameLengthLines = 0x0d14;/*3348*/
		}else if(pConfig->Resolution == ISI_RES_1296_972P15) {
		  usFrameLengthLines = 0x1170;/*4464*/
		}else if(pConfig->Resolution == ISI_RES_1296_972P10) {
		  usFrameLengthLines = 0x1a28;/*6696*/
		}
	  ...
	  case ISI_RES_TV1080P24:
		if (set2Sensor == BOOL_TRUE) {
		  if (res_no_chg == BOOL_FALSE) {
			if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_1920x1080)) != RET_SUCCESS){
			  result = RET_FAILURE;
			  TRACE( Sensor_ERROR, "%s: failed to set two lane ISI_RES_1920_1080 \n", __FUNCTION__ );
			}
		  }
		}
		break;
	  case ISI_RES_VGAP30:
		if (set2Sensor == BOOL_TRUE) {
		  if (res_no_chg == BOOL_FALSE) {
			if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_640x480)) != RET_SUCCESS){
			  result = RET_FAILURE;
			  TRACE( Sensor_ERROR, "%s: failed to set two lane ISI_RES_640x480 \n", __FUNCTION__ );
			}
		  }
		}
		break;
	  ...
	  if (set2Sensor == BOOL_TRUE) {
		if (res_no_chg == BOOL_FALSE) {
		  if((result = IsiRegDefaultsApply((IsiSensorHandle_t)pSensorCtx, Sensor_g_twolane_resolution_1280x960)) != RET_SUCCESS){
			result = RET_FAILURE;
			  TRACE( Sensor_ERROR, "%s: failed to set two lane ISI_RES_1280x960 \n", __FUNCTION__ );
		  }
		}
	  }
	  break;
	...
	pSensorCtx->LineLengthPck    = usLineLengthPck;/*0x380c&&0x380d?*/
	pSensorCtx->FrameLengthLines = usFrameLengthLines;/*0x380e&&0x380f?*/
	#else
	pSensorCtx->IsiSensorMipiInfo.ulMipiFreq = 24/1/6*105/1;//????
	rVtPixClkFreq = pSensorCtx->IsiSensorMipiInfo.ulMipiFreq*1000000/5;//??
	pSensorCtx->VtPixClkFreq     = rVtPixClkFreq;

	Sensor_IsiRegReadIss(  pSensorCtx, 0x380c, &temp1 );
	Sensor_IsiRegReadIss(  pSensorCtx, 0x380d, &temp2 );
	pSensorCtx->LineLengthPck    = ((temp1<<8) | temp2);/*0x380c&&0x380d?*/

	Sensor_IsiRegReadIss(  pSensorCtx, 0x380e, &temp1 );
	Sensor_IsiRegReadIss(  pSensorCtx, 0x380f, &temp2 );
	temp3 = (((temp1&0x7f)<<8) | temp2);/*0x380e&&0x380f?*/
	pSensorCtx->FrameLengthLines = temp3;
#endif
	...
	TRACE( Sensor_INFO, "%s AecMaxIntegrationTime:%f(****************exit): Resolution %dx%d@%dfps  MIPI %dlanes  res_no_chg: %d   rVtPixClkFreq: %f, ulMipiFreq=%u\n", __FUNCTION__,
		...
		pSensorCtx->IsiSensorMipiInfo.ulMipiFreq);
...
)
{
  ...
  TRACE( Sensor_INFO, "%s: ISI_RES_W_GET(pConfig->Resolution)=%d, ISI_RES_W_GET(pSensorCtx->Config.Resolution)=%d\n", __FUNCTION__
	, ISI_RES_W_GET(pConfig->Resolution)
	, ISI_RES_W_GET(pSensorCtx->Config.Resolution));
  TRACE( Sensor_INFO, "%s: res_no_chg=%d, fixme, why NOT uset it?\n", __FUNCTION__, res_no_chg);
...
)
{
  ...
  result = IsiRegDefaultsVerify( pSensorCtx, Sensor_g_aRegDescription );
	...
	if (result == RET_SUCCESS)
	{
	TRACE( Sensor_ERROR, "%s: Configured\n", __FUNCTION__);
...
static void ov5695StreamOn(IsiSensorHandle_t   handle, bool_t enable)
{

  Sensor_Context_t *pSensorCtx = (Sensor_Context_t *)handle;

	if (enable == BOOL_TRUE) {
	TRACE( Sensor_INFO, "%s: enable\n", __FUNCTION__);
	Sensor_IsiRegWriteIss ( pSensorCtx, 0x4202, 0x00);
	Sensor_IsiRegWriteIss ( pSensorCtx, 0x4800, 0x24);
  } else {
	Sensor_IsiRegWriteIss ( pSensorCtx, 0x4202, 0x0f);
	Sensor_IsiRegWriteIss ( pSensorCtx, 0x4800, 0x21);
  }
}
...
)
{
  ...
  TRACE( Sensor_INFO, "%s (enter)  on = %d\n", __FUNCTION__,on);
	if ( pSensorCtx == NULL )
	{
	TRACE( Sensor_INFO, "%s: err, pSensorCtx is NULL\n", __FUNCTION__);
		return ( RET_WRONG_HANDLE );
	}

	if ( (pSensorCtx->Configured != BOOL_TRUE) || (pSensorCtx->Streaming == on) )
	{
	TRACE( Sensor_INFO, "%s: err,no Configured, but now on\n", __FUNCTION__);
		return RET_WRONG_STATE;
	}
	...
	TRACE( Sensor_INFO, "%s: read reg return result=%d, RegValue=0x%x\n", __FUNCTION__, result, RegValue);

		RETURN_RESULT_IF_DIFFERENT( RET_SUCCESS, result );
	result = Sensor_IsiRegWriteIss ( pSensorCtx, Sensor_MODE_SELECT, (RegValue | 0x01U) );

	TRACE( Sensor_INFO, "%s: write reg return result=%d\n", __FUNCTION__, result);
...
/*
int static gpio_export(unsigned int gpio)
{
  int fd, len;
  char buf[64];
  ...
  if(fd<0) {
	printf("%s: open gpio%d export err\n", __func__, gpio);
	return fd;
  }
...
*/
int static gpio_set_value_sk(unsigned int gpio, unsigned int value)
{
  int fd;
  char buf[64];
...
void static IsiSensorSetPowerIss_sk_tmp()
{
  g_gpio_drv.pfn_export(3);

  gpio_set_value_sk(3, 0);
  osSleep( 10 );
  gpio_set_value_sk(2, 1);
  osSleep( 10 );
  gpio_set_value_sk(3, 1);
  osSleep( 10 );

  gpio_set_value_sk(2, 0);
  osSleep( 10 );
  gpio_set_value_sk(2, 1);
  osSleep( 10 );
}

static RESULT OV5695PowerCtrl(uint8_t on)
{
	RESULT ret = 0;
	ret |= g_gpio_drv.pfn_set_output(GPIO_PIN_OV5695_AVDD, on);
	ret |= g_gpio_drv.pfn_set_output(GPIO_PIN_OV5695_DVDD, on);
	ret |= g_gpio_drv.pfn_set_output(GPIO_PIN_OV5695_IO, on);
	return ret;
}

static RESULT OV5695Reset(void)
{
	return g_gpio_drv.pfn_reset(OV5695_RST);
}
...
)
{
	...
	pSensorCtx->Configured = BOOL_FALSE;
	pSensorCtx->Streaming  = BOOL_FALSE;
  TRACE( Sensor_INFO, "%s how to do\n", __FUNCTION__);
#ifdef ISP_FPGA
  HalSensorReset(pSensorCtx->IsiCtx.HalHandle, pSensorCtx->IsiCtx.HalDevID, true);
	IsiSensorSetPowerIss_sk_tmp();
#else
	OV5695PowerCtrl(on);
	OV5695Reset();
#endif
...
//    TRACE( Sensor_REG_DEBUG, "%s (IsiGetNrDatBytesIss %d 0x%08x 0x%08x)\n", __FUNCTION__, NrOfBytes, address, value);
  ...
  #else
	result = IsiI2cWriteRegister( handle, address, (uint8_t *)(&value), NrOfBytes, BOOL_TRUE );
  #endif
	//TRACE( Sensor_ERROR, "%s (exit: 0x%08x 0x%08x)\n", __FUNCTION__, address, value);
	  ...
	  case SUPPORT_MIPI_TWO_LANE:
	  {
		...
		switch(Resolution)
		{
		  ...
		  case ISI_RES_VGAP30:
			...
			break;
		  case ISI_RES_1280_960P30:
			AFPSCHECKANDADD( ISI_RES_1280_960P30);
			break;
		}
		break;
	  }
...
)
{
	...
	TRACE( Sensor_INFO, "%s: (enter)\n", __FUNCTION__);
  TRACE( Sensor_INFO, "%s: should be here?\n", __FUNCTION__);
...
static RESULT Sensor_IsiGetSensorI2cInfo(sensor_i2c_info_t** pdata)
{
  ...
  *pdata = pSensorI2cInfo;
  ...
  TRACE( Sensor_ERROR,  "%s: done, i2c_addr=0x%x, sensor_drv_version=%d(0x%x)\n",__FUNCTION__
	  , pSensorI2cInfo->i2c_addr
	  , pSensorI2cInfo->sensor_drv_version, pSensorI2cInfo->sensor_drv_version);
